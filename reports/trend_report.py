...
def load_data():
    """Load competitor listings only: rows present in BOTH listings and
    competitor_scores for the same date (the scores table is the
    authoritative record of what passed the competitor filters)."""
    ...
    for d in days:
        rows = db.execute(
            ...
            "WHERE l.scrape_date=?", (d,)).fetchall()
...
<div class="wrap">
  ...
  <div class="note">Rec. price = suggested price for our $250 listing from that day's daily report (comp-set anchored).</div>
  ...
  <div class="note" style="margin-top:24px">Competitors only: every listing shown passed the config/areas.json
    competitor filters (3–5 BR houses/townhomes, $150–600, ≤20 km) and has a matching competitor_scores row
    for that date. Source: data/market.db · generated by reports/trend_report.py — re-run after any daily run.</div>
</div>
</body>
</html>
"""
